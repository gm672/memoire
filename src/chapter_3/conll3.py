...
####
# The function remove_punct was added to the original script.
####
	...
	with open(path, encoding='utf-8') as f:
		...
		for li in f:
			...
			if li:
				...
				conlltext = remove_punct(conlltext)
        ...
        if el[0] != "#":
            ...
            if tokens[3] == "PUNCT":
                ...
                if re.search('-',tokens[0]):
                    ...
                    ni = int(tokens[0]) + diff
                    new = str(ni)
                dic[tokens[0]]=new
                lNoPunct.append(tokens)
    final = []
    final.append(lines[1])
    for el in lNoPunct:
        el[0] = dic[el[0]]
        el[6] = dic[el[6]]
        final.append('\t'.join(map(str, el)))
    return '\n'.join(map(str, final))
